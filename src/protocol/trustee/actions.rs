...
impl<E: Element, G: Group<E>> Trustee<E, G> {
    pub(crate) fn check_config<B: BulletinBoard<E, G>>(&self, action: Act, self_index: u32, 
        cfg_h: Hash, board: &mut B) -> Result<(), TrusteeError> {
    ...
    pub(crate) fn post_share<B: BulletinBoard<E, G>>(&self, action: Act, self_index: u32, cfg_h: Hash, 
        contest: u32, board: &mut B) -> Result<(), TrusteeError> {
        
        info!(">> Action: Computing shares (contest=[{}], self=[{}])..", contest, self_index);
    ...
    pub(crate) fn combine_shares<B: BulletinBoard<E, G>>(&self, action: Act, self_index: u32, cfg_h: Hash, 
        contest: u32, share_hs: Hashes, board: &mut B) -> Result<(), TrusteeError> {
        
        info!(">> Action: Combining shares (contest=[{}], self=[{}])..", contest, self_index);
        let cfg = board.get_config(cfg_h)?
            .ok_or(TrusteeError::Msg("Could not find cfg".to_string()))?;
        let hashes = clear_zeroes(&share_hs);
        assert!(hashes.len() == cfg.trustees.len());
        let pk = self.get_pk(board, hashes, &cfg.group, contest)
    ...
    pub(crate) fn check_pk<B: BulletinBoard<E, G>>(&self, action: Act, self_index: u32, cfg_h: Hash, 
        contest: u32, pk_h: Hash, share_hs: Hashes, board: &mut B) -> Result<(), TrusteeError> {
        
        info!(">> Action: Verifying pk (contest=[{}], self=[{}])..", contest, self_index);
        let cfg = board.get_config(cfg_h)?
            .ok_or(TrusteeError::Msg("Could not find cfg".to_string()))?;
        let hashes = clear_zeroes(&share_hs);
        assert!(hashes.len() == cfg.trustees.len());
    ...
    pub(crate) fn mix<B: BulletinBoard<E, G>>(&self, action: Act, self_index: u32, cfg_h: Hash, 
        contest: u32, ballots_h: Hash, pk_h: Hash, board: &mut B) -> Result<(), TrusteeError> {
    
        info!(">> Computing mix (contest=[{}], self=[{}])..", contest, self_index);
        let cfg = board.get_config(cfg_h)?
            .ok_or(TrusteeError::Msg("Could not find cfg".to_string()))?;
        let ciphertexts = self.get_mix_src(board, contest, self_index, ballots_h)
            .ok_or(TrusteeError::Msg("Could not find source ciphertexts".to_string()))?;
        let pk = board.get_pk(contest, pk_h)?
    ...
    pub(crate) fn check_mix<B: BulletinBoard<E, G>>(&self, action: Act, self_index: u32, cfg_h: Hash, 
        contest: u32, trustee:u32, mix_h: Hash, ballots_h: Hash, pk_h: Hash, 
        board: &mut B) -> Result<(), TrusteeError> {
        
        let cfg = board.get_config(cfg_h)?
            .ok_or(TrusteeError::Msg("Could not find cfg".to_string()))?;
        
        info!(">> Action:: Verifying mix (contest=[{}], self=[{}])..", contest, self_index);

        let mix = board.get_mix(contest, trustee, mix_h)?
            .ok_or(TrusteeError::Msg("Could not find mix".to_string()))?;
        
        let ciphertexts = self.get_mix_src(board, contest, trustee, ballots_h)
            .ok_or(TrusteeError::Msg("Could not find source ciphertexts".to_string()))?;
        let pk = board.get_pk(contest, pk_h)?
            .ok_or(TrusteeError::Msg("Could not find pk".to_string()))?;
        let group = &cfg.group;
        ...
        let ss = SignedStatement::mix(&cfg_h, &mix_h, &ballots_h, Some(trustee), contest, &self.keypair);
    ...
    pub(crate) fn partial_decrypt<B: BulletinBoard<E, G>>(&self, action: Act, self_index: u32, cfg_h: Hash, 
        contest: u32, mix_h: Hash, share_h: Hash, board: &mut B) -> Result<(), TrusteeError> {
        
        info!(">> Action: Computing partial decryptions (contest=[{}], self=[{}])..", contest, self_index);
        let now_ = std::time::Instant::now();
        
        let cfg = board.get_config(cfg_h)?
            .ok_or(TrusteeError::Msg("Could not find cfg".to_string()))?;
        
        let mix = board.get_mix(contest, (cfg.trustees.len() - 1) as u32, mix_h)?
            .ok_or(TrusteeError::Msg("Could not find mix".to_string()))?;
        
        let share = board.get_share(contest, self_index, share_h)?
    ...
    pub(crate) fn combine_decryptions<B: BulletinBoard<E, G>>(&self, action: Act, self_index: u32, cfg_h: Hash, 
        contest: u32, decryptions_hs: Hashes, mix_h: Hash, share_hs: Hashes, 
        board: &mut B) -> Result<(), TrusteeError> {
        
        let cfg = board.get_config(cfg_h)?
            .ok_or(TrusteeError::Msg("Could not find cfg".to_string()))?;
        info!(">> Action: Combining decryptions (contest=[{}], self=[{}])..", contest, self_index);
        let now_ = std::time::Instant::now();
        let d_hs = clear_zeroes(&decryptions_hs);
        let s_hs = clear_zeroes(&share_hs);
        let pls = self.get_plaintexts(board, contest, d_hs, mix_h, s_hs, &cfg)
    ...
    pub(crate) fn check_plaintexts<B: BulletinBoard<E, G>>(&self, action: Act, self_index: u32, cfg_h: Hash, 
        contest: u32, plaintexts_h: Hash, decryptions_hs: Hashes, mix_h: Hash, share_hs: Hashes, 
        board: &mut B) -> Result<(), TrusteeError> {
        
        let cfg = board.get_config(cfg_h)?
            .ok_or(TrusteeError::Msg("Could not find cfg".to_string()))?;                    
        info!(">> Action: Checking plaintexts (contest=[{}], self=[{}])", contest, self_index);
        let now_ = std::time::Instant::now();
        let s_hs = clear_zeroes(&share_hs);
        let d_hs = clear_zeroes(&decryptions_hs);
        let pls = self.get_plaintexts(board, contest, d_hs, mix_h, s_hs, &cfg)
            .ok_or(TrusteeError::Msg("Could not build plaintexts".to_string()))?;
        let rate = pls.len() as f32 / now_.elapsed().as_millis() as f32;
        let pls_board = board.get_plaintexts(contest, plaintexts_h)?
